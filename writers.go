...
One{{goify $rmt.TypeName true}}{{if eq $vname "default"}}{{else}}{{goify $vname true}}{{end}} (ctx context.Context{{ if $ut.DynamicTableName}}, tableName string{{ end }}, id int{{range $nm, $bt := $ut.BelongsTo}},{{goify $bt.ModelName false}}id int{{end}}) (*{{goify $vpn false}}.{{goify $rmt.TypeName true}}{{if eq $vname "default"}}{{else}}{{goify $vname true}}{{end}}, error)
{{ end }}
...
func (m *{{$ut.ModelName}}DB) List{{$ut.TypeName}}(ctx context.Context{{ if $ut.DynamicTableName}}, tableName string{{ end }}) []{{$ut.ModelName}}{
	...
	if err != nil && err != gorm.RecordNotFound {
		goa.Error(ctx, "error listing {{$ut.ModelName}}", goa.KV{"error", err.Error()})
		return objs
	}
...
func (m *{{$ut.ModelName}}DB) Delete(ctx context.Context{{ if $ut.DynamicTableName }}, tableName string{{ end }}, {{$ut.PKAttributes}})  error {
	...
	{{ end }}
	if err != nil {
		goa.Error(ctx, "error retrieving {{$ut.ModelName}}", goa.KV{"error", err.Error()})
		return  err
	}
