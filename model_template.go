...
{{ $pieces := split $bt ":" }} {{ $lowertype := index $pieces 1  }} {{ $lower := title $lowertype }}
func (m *{{$typeName}}DB) Delete{{index $pieces 1}}(ctx *app.Delete{{index $pieces 1}}{{$typeName}}Context)  error {
	var obj {{$typeName}}
	err := m.DB.Delete(&obj, ctx.{{demodel $typeName}}ID).Error
	if err != nil {
		ctx.Logger.Error(err.Error())
