...
func StorageDefinition(res *design.UserTypeDefinition) string {
	...
	if assoc, ok := res.Metadata["github.com/bketelsen/gorma#many2many"]; ok {
		...
		for _, child := range children {
			...
			associations = associations + "Add" + pieces[1] + "(ctx *app.Create" + strings.ToLower(pieces[1]) + DeModel(res.TypeName) + "Context) (" + pieces[1] + " error)\n"
			associations = associations + "Delete" + pieces[1] + "(ctx *app.Create" + strings.ToLower(pieces[1]) + DeModel(res.TypeName) + "Context) error \n"
		}
	}
	return associations
}
